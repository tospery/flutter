...
import 'package:hi_base/src/extension/int.dart';

extension StringHiBaseEx on String {
  String get base64Decoded => convert.utf8.decode(
    convert.base64.decode(replaceAll(RegExp(r'\s+'), '')),
  );
  ...
  bool get isValidHttpUrl =>
      ['https', 'http'].contains(this.toUri()?.scheme.toLowerCase());

  bool get isValidImageUrl =>
      ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'svg'].contains(
        this
            .toUri()
            ?.pathSegments
            .lastOrNull
  ...
  bool get isValidPdfUrl => ['pdf'].contains(
    this.toUri()?.pathSegments.lastOrNull?.split('.').lastOrNull?.toLowerCase(),
  );

  bool get isValidMarkdownUrl => ['md', 'mdx'].contains(
    this.toUri()?.pathSegments.lastOrNull?.split('.').lastOrNull?.toLowerCase(),
  );
